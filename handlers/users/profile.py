...
async def profile_message(message: types.Message):
    data = db_users.deanon(message.from_user.id)

    teht = check(data)

    await bot.send_message(message.from_user.id, users.text_profile.format(data.id, data.username,
                                                                     round((data.win_count / data.all_count) * 100,
                                                                           2) if data.all_count else 0, data.date,
